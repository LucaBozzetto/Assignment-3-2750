...
void insertSorted(List *list, void *toBeAdded){
	...
	// (list->length)++; //buggy, insertBack and insertFront increment list->lenght already, this should be moved down
	...
	while (currNode != NULL){
		if (list->compare(toBeAdded, currNode->data) <= 0){
			...
			(list->length)++; //increments after the insertion.
...
ListIterator createIterator(List* list){
	ListIterator tmpIter;
	tmpIter.current = list->head;
	return tmpIter;
...
void clearList(List* list){
	...
	Node *nodeToBeFreed = iter.current;
